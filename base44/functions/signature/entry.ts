...
Deno.serve(async (req) => {
  ...
  if (!email) {
    return new Response('', { status: 200, headers: { ...CORS, 'Content-Type': 'text/html' } });
  }

  const base44 = createClientFromRequest(req);
  const [firmList, allStaff] = await Promise.all([
    base44.asServiceRole.entities.FirmSettings.list(),
    base44.asServiceRole.entities.StaffSignature.list(),
  ]);
  ...
  return new Response(renderSignature(staff, firm), {
    status: 200,
